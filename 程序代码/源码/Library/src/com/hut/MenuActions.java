...
import com.hut.iframe.BookAddIFrame;
import com.hut.iframe.BookBackIFrame;
import com.hut.iframe.BookBorrowIFrame;
import com.hut.iframe.BookModiAndDelIFrame;
import com.hut.iframe.BookTypeAddIFrame;
import com.hut.iframe.BookTypeModiAndDelIFrame;
import com.hut.iframe.GengGaiMiMa;
import com.hut.iframe.ReaderAddIFrame;
import com.hut.iframe.ReaderModiAndDelIFrame;
import com.hut.iframe.newBookCheckIFrame;
import com.hut.iframe.newBookOrderIFrame;
...
public class MenuActions {
	...
	public static CheckAndAcceptNewBookAction NEWBOOK_CHECK_ACCEPT;// 新书验收动作
	...
	//管理员密码更改
	/*
	 * AbstractAction定义了一些标准行为，比如 Action 对象属性（icon、text 和 enabled）的 get 和 set 方法
	 */
	private static class PasswordModiAction extends AbstractAction{
		PasswordModiAction(){
			/*
			 * Action  接口 提供 ActionListener 接口的一个有用扩展，以便若干控件访问相同的功能
			 */
			putValue(Action.NAME, "更改口令");
			putValue(Action.LONG_DESCRIPTION, "修改当前用户密码");
			putValue(Action.SHORT_DESCRIPTION, "更换口令");  //在更改口令中显示文字
		}
		...
		private static class  BookSearchAction extends AbstractAction {
			BookSearchAction(){
				super("图书搜索", null);
				putValue(Action.LONG_DESCRIPTION,"搜索入库的图书");
				putValue(Action.SHORT_DESCRIPTION, "用户添加");
			}
